...
export const convertMobDroptoJSON = (mobDrop: any) => {
    ...
    mobDrop
            ...
            .map(el => {
                ...
                if(el.includes(`\t1`) && !el.includes('Mob') && !el.includes('Kill_drop') && !el.includes('Level_limit')){
                    startReadDrop = true;
                }
                if(startReadDrop){
                    ...
                    tempResult.push(
                        {
                            mob: parseInt(actualMob), 
                            item: {
                                id: parseInt(itemTab[2]), 
                                amount: parseInt(itemTab[3]), 
                                chance: parseFloat(itemTab[4])
                            }
                        }) 
                }
            })
